...
def main():
    ...
    if args.do_train and is_main_process():
        ...
        log("  Num examples = %d", len_train_features)
        log("  Num Epochs = %d", args.num_train_epochs)
        log("  Instantaneous batch size per GPU = %d", args.train_batch_size)
        log(
            "  Total train batch size (w. parallel, distributed & accumulation) = %d",
            args.train_batch_size
            * get_world_size(),
        )
        log("  Total optimization steps = %d", total_train_steps)
    ...
    for epoch in range(EPOCHS):
        if args.do_train:
            ...
            for iter, inputs in enumerate(epoch_iterator):
                ...
                if iter % 100 == 0:
                    if is_main_process():
                        log("\nEpoch: {:03d}, Step:{:6d}, Loss:{:12.8f}, Perf:{:5.0f}".format(epoch, iter, loss_value,
        ...
        if args.do_predict and (args.evaluate_during_training or epoch == args.num_train_epochs - 1):
            ...
            if is_main_process():
                ...
                log("  Num examples = %d", total_dev_steps)
                log("  Batch size = %d", args.predict_batch_size)
